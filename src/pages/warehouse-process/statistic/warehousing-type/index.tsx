...
import { ProposalType, WarehousingType } from '@/services/swr/statistic.twr';
...
const WarehousingTypeChart = ({ ...props }: Props) => {
    ...
    const [dataWarehousingType, setDataWarehousingType] = useState<any>();

    const isDark = useSelector((state: IRootState) => state.themeConfig.theme === 'dark' || state.themeConfig.isDarkMode);
    const isRtl = useSelector((state: IRootState) => state.themeConfig.rtlClass) === 'rtl' ? true : false;

    // get data
    const { data: warehousingType } = WarehousingType({ ...router.query });

    useEffect(() => {
        var a: any = [];
        var b: any = [];
        (warehousingType?.data.map((item: any) => {
            return a.push(item.count) && b.push(item.type)
        }))
        setDataWarehousingType(
            {
                series: [{ data: a }],
                name: b
            }
    ...
    const options: any = {
        ...
        xaxis: {
            categories: dataWarehousingType?.name || [],
            axisBorder: {
                show: true,
...
export default WarehousingTypeChart;
